...
But this is not very *nice* because there is a fair bit of repetition. Repeating
...
Here we are going to cover 5 of the most commonly used functions as well as using
...
If we open up `year_country_gdp` we will see that it only contains the year,
country and gdpPercap. Above we used 'normal' grammar, but the strengths of
`dplyr` lie in combining several functions using pipes. Since the pipes grammar
is unlike anything we have seen in R before, let's repeat what we have done above
using pipes.
...
we do not specify which data object we use in the `select()` function since in
...
be done with base R, but up to now we have not done that since we would have to
...
That is already quite powerful, but it gets even better! You are not limited to defining 1 new variable in `summarize()`.
...
to `ggplot()`. Because `%>%` replaces the first argument in a function we do not
