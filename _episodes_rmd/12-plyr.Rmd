...
A common task you will encounter when working with data, is that you will want to
...
But this is not very *nice*. Yes, by using a function, you have reduced a
...
`apply` family of functions. While R's built in functions do work, we are
...
They are named by their input data type and represent null output by a `_` (see
table)
...
- The third argument is the function we want to apply to each grouping of the
  ...
  function: we have not defined it elsewhere, and it has no name. It only exists
