...
# Silently set seed for random number generation, so we do not have to explain it
set.seed(10)
```

Often when we are coding we want to control the flow of our actions. This can be done
...
> can very quickly slow to a crawl. It is much better to define
...
>
> As an example, here is a while loop
> that generates random numbers from a uniform distribution (the `runif()` function)
> between 0 and 1 until it gets one that is less than 0.1.
>
> ~~~
> z <- 1
> while(z > 0.1){
...
> that you do not end up stuck in an infinite loop because your condition is always met and hence the while statement never terminates.
{: .callout}
