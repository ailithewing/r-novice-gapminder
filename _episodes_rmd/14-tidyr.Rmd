...
First install the packages if you have not already done so (you probably
...
Until now, we have been using the nicely formatted original gapminder dataset, but
...
We will load the data file and look at it. Note: we do not want our continent and
...
Now we have got an intermediate data frame `gap_normal` with the same dimensions as
...
We are almost there, the original was sorted by `country`, then
`year`.
...
That is great! We have gone from the longest format back to the intermediate and we
did not introduce any errors in our code.
...
`continent`,`country`,and we have defined variable names. We are now ready to
pipe in `pivot_wider()`
