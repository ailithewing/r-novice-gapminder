...
The report can be one long stream, so tall figures that would not ordinarily fit on
...
to produce. In this case, we are creating an html document.
...
You can delete any of those fields if you do not want them
included. The double-quotes are not strictly _necessary_ in this case.
They are mostly needed if you want to include a colon in the title.
...
For now, let's delete all of the stuff that is there and write a bit of
markdown.
...
Often there will be particular options that you will want to use
...
Do not let these in-line chunks get split across lines.
...
> > 
> > Here is some inline code to determine that 2 + 2 = `` `r
> > 2+2` ``.
> > 
> {: .solution}
{: .challenge}
...
> software (you will only need to do this once) and then when you knit to pdf `tinytex`
